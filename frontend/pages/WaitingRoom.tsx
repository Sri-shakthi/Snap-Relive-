...
const WaitingRoom: React.FC = () => {
  ...
  const steps = [
    { id: 1, title: 'Photographer clicks photos', status: 'completed' },
    { id: 2, title: 'Photos upload to cloud', status: 'completed' },
    { id: 3, title: 'AI matching your face', status: 'active' },
    { id: 4, title: 'Gallery updates', status: 'pending' },
  ];
      ...
      <div className="flex-1 flex flex-col items-center py-10 space-y-10">
        <div className="text-center space-y-4">
          ...
          <p className="text-stone-500 max-w-xs mx-auto">
            When the photographer uploads photos, they’ll appear here in about 5 minutes.
          </p>
        </div>
              ...
              <div key={step.id} className="flex items-start gap-4">
                <div className="relative">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                    step.status === 'completed' ? 'bg-stone-900 text-white' : 
                    step.status === 'active' ? 'bg-stone-100 text-stone-900 border-2 border-stone-900' : 
                  ...
                  {idx < steps.length - 1 && (
                    <div className={`absolute top-8 left-4 w-0.5 h-6 ${step.status === 'completed' ? 'bg-stone-900' : 'bg-stone-100'}`} />
                  )}
                </div>
                <div className="pt-1">
